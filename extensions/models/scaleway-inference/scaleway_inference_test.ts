...
// --- deploymentId is optional: create provisions it, others require it ------

Deno.test("create works with no deploymentId set (no placeholder needed)", async () => {
  const { deploymentId: _omitted, ...noId } = G;
  ...
  await withMockedFetch(
    ...
    () =>
      model.methods.create.execute({
        name: "provisioned-deployment",
        modelId: "77777777-7777-7777-7777-777777777777",
        nodeType: "L4",
        acceptEula: true,
        publicEndpoint: true,
        disableAuth: false,
  ...
  assertEquals(writes[0].data.name, "provisioned-deployment");
});

for (const method of ["sync", "update", "delete"] as const) {
  Deno.test(`${method} fails fast with an actionable error when deploymentId is absent`, async () => {
    const { deploymentId: _omitted, ...noId } = G;
    const { ctx, writes } = makeContext(noId);
    let fetched = false;
    ...
    assert(err !== null, `${method} should throw when deploymentId is missing`);
    assertStringIncludes(err.message, "deploymentId");
