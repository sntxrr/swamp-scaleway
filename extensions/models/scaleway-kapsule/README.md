...
The cluster **kubeconfig** contains cluster-admin credentials. The `get-kubeconfig`
method stores the base64 `content` in a dedicated `kubeconfig` resource whose
`content` field is marked **sensitive**: the raw kubeconfig is never logged and
is never written into a normal `cluster` or `pool` snapshot. The `secretKey`
global argument is likewise sensitive and wired from a vault.
...
method is not one of the auto-checked verb names (`create`/`update`/`delete`/`action`),
so its pre-flight does not fire automatically — validate the region on a prior
`create`/`update`, or run `sync` first.
...
| `clusterId` | yes      | —                          | ID of the Kapsule cluster this model manages           |
