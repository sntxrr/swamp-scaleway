...
in the `X-Auth-Token` header and is never logged or written into a snapshot. Job
**environment variables** — both those passed to `create`/`update` and the
...
| `jobDefinitionId` | conditional | —                          | ID of the job definition this model manages. Required by every method except `create`, which provisions it |
