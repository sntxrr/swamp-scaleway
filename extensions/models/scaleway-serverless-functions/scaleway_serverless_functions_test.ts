...
// --- functionId is optional: create provisions it, others require it --------

Deno.test("create works with no functionId set (no placeholder needed)", async () => {
  const { functionId: _omitted, ...noId } = G;
...
for (const method of ["sync", "update", "action", "delete"] as const) {
  Deno.test(`${method} fails fast with an actionable error when functionId is absent`, async () => {
    const { functionId: _omitted, ...noId } = G;
    const { ctx, writes } = makeContext(noId);
    let fetched = false;
    ...
    assert(err !== null, `${method} should throw when functionId is missing`);
    assertStringIncludes(err.message, "functionId");
