...
// --- lbId is optional: create provisions it, other methods require it -------

Deno.test("create works with no lbId set (no placeholder needed)", async () => {
  const { lbId: _omitted, ...noId } = G;
  ...
  await withMockedFetch(
    ...
    () =>
      model.methods.create.execute(
        { name: "provisioned-lb", type: "LB-S" },
        ctx,
      ),
  );
  // The new LB ID comes from the API response, not from a preset globalArg.
  ...
  assertEquals(writes[0].data.name, "provisioned-lb");
});

/** Every method that operates on an existing LB, with valid arguments. */
const guardedMethods: Array<[string, (ctx: AnyCtx) => Promise<unknown>]> = [
  ...
  ["delete", (ctx) => model.methods.delete.execute({ releaseIp: false }, ctx)],
  ["list-backends", (ctx) => model.methods["list-backends"].execute({}, ctx)],
  ["list-frontends", (ctx) => model.methods["list-frontends"].execute({}, ctx)],
];

for (const [method, run] of guardedMethods) {
  Deno.test(`${method} fails fast with an actionable error when lbId is absent`, async () => {
    const { lbId: _omitted, ...noId } = G;
    const { ctx, writes } = makeContext(noId);
    let fetched = false;
    ...
    assert(err !== null, `${method} should throw when lbId is missing`);
    assertStringIncludes(err.message, "lbId");
