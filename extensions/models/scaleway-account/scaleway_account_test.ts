...
// --- projectId is optional: create provisions it, others require it ---------

Deno.test("create works with no projectId set (no placeholder needed)", async () => {
  const { projectId: _omitted, ...noId } = G;
  ...
  await withMockedFetch(
    ...
    () => model.methods.create.execute({ name: "new-project" }, ctx),
  );
...
for (const method of ["sync", "update", "delete"] as const) {
  Deno.test(`${method} fails fast with an actionable error when projectId is absent`, async () => {
    const { projectId: _omitted, ...noId } = G;
    const { ctx, writes } = makeContext(noId);
    let fetched = false;
    ...
    assert(err !== null, `${method} should throw when projectId is missing`);
    assertStringIncludes(err.message, "projectId");
